...
class TicketFrag(BaseViewer):
    ...
    def get_all_assigned(self):
        ...
        tlist = self.tickets.get_assigned(user_id)
        env = self.env.update(tlist=tlist)
        self.response = self.render(self._template, self.env)

    def get_all_delegated(self):
        user_id = self._getuserid()
        tlist = self.tickets.get_delegated(user_id)
        env = self.env.update(tlist=tlist)
        self.response = self.render(self._template, self.env)

    def get_all_unread(self):
        user_id = self._getuserid()
        tlist = self.tickets.get_unread(user_id)
        env = self.env.update(tlist=tlist)
        self.response = self.render(self._template, self.env)
        
    def get_all_pending(self):
        user_id = self._getuserid()
        tlist = self.tickets.get_pending(user_id)
        env = self.env.update(tlist=tlist)
        self.response = self.render(self._template, self.env)
        
    def get_all_closed(self):
        user_id = self._getuserid()
        tlist = self.tickets.get_closed(user_id)
        env = self.env.update(tlist=tlist)
        self.response = self.render(self._template, self.env)
...
class BaseTicketViewer(BaseViewer):
    ...
    def main_tickets_view(self):
        ...
        for tkt_type in tkt_types:
            ...
            route = 'msl_tktfrag'
            context = 'all_%s_tickets' % tkt_type
            url = self.request.route_url(route, context=context, id=t)
